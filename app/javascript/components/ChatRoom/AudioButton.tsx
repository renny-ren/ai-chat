...
const AudioButton: React.FC<AudioButtonProps> = ({ message }) => {
  ...
  const handlePlay = async (message) => {
    ...
    const audioUrl = AliyunAPI.fetchTtsStream(token, message.content)
    const a = new Audio(audioUrl)
    setAudio(a)
    a.play()
    a.onplaying = () => setPlayingId(message.id)
    a.onended = () => setPlayingId(0)
  }
  const handlePlayThrottled = _.throttle(handlePlay, 5000, { trailing: false })

  return (
    <button className="relative -left-2.5 -top-1 self-end" onClick={() => handlePlayThrottled(message)}>
