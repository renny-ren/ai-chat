...
const DownloadButton: React.FC<DownloadButtonProps> = ({ messages, conversationId }) => {
  ...
  const onClickDownload = (type = "md") => {
    fetchConversation().then((data) => {
      ...
      downloadLink.download = `${data.conversation.title} - 智言智语 AI 对话记录.${type}`
    ...
    <div>
      <Tooltip title="导出对话内容">
        <button
          type="button"
          onClick={openModal}
          className="outline-none inline-flex px-2 py-1 text-xs text-gray-600 transition-colors duration-300 transform rounded-md dark:text-gray-200 dark:border-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700"
        >
