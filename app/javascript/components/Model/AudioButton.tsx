...
const AudioButton: React.FC<AudioButtonProps> = ({ voice = "", content = "" }) => {
  ...
  const handlePlay = async () => {
    setLoading(true)
    const src = await tts.fetchStream(content || "你好，很高兴认识你。请输入自我介绍，我会朗读自我介绍里面的内容", voice)
    playAudio(src)
    setLoading(false)
  }

  const handlePlayThrottled = _.throttle(handlePlay, 3000, { trailing: false })
  ...
  return (
    <>
      ...
      <button type="button" className="ml-2" onClick={onClickAudio}>
        {loading ? (
          <Spinner />
        ) : (
          <>
